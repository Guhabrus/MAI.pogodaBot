...
#---------------------------------phareses
end_phrase='Я рад Вам помочь 😀'
enter_phrase='Привет!🙃  \n Я бот-помощник на основе искусственного интеллекта. Помогу прдесказть тебе погоду!\n Выбери команду 👇 '
help_phrase='Для того, чтобы начать работу с ботом напишите 👇 \n /start\n Прейти в раздел меню введите 👇 \n /menu'
#input_cor_phrase='Введите координаты долготы и широты через пробел'
input_data_phrase='Загрузите csv файл с Вашими методанными✈️'
...
def handle_start(message):
    ...
    markup.row('/Загрузить данные csv📎')
    markup.row('/Продолжить с уже загруженными данными➡️')
...
def handle_start(message):
    ...
    markup.add(phone)
    bot.send_message(message.chat.id,'Подтвердите свой номер телефона' , reply_markup=markup)
...
def handle_start(message):
    ...
    markup.row('/Ставрополь 3 дня предсказаний➡️')
    markup.row('/Меню')
...
def location(message):
    if message.location is not None:
        ...
        markup =telebot.types.ReplyKeyboardMarkup(True)
        markup.row('/1 день предсказаний➡️')
        markup.row('/3 дня предсказаний➡️')
        markup.row('/Меню')
        bot.send_message(message.chat.id, 'Выбериете длительность предсказаний⏲️',reply_markup=markup)
    else:
        markup =telebot.types.ReplyKeyboardMarkup(True)
...
def handle_start(message):
    ...
    if personal_info[message.chat.id]['cordinates']==None and personal_info[message.chat.id]['file']==None:
        ...
        markup2.row('/1 день предсказаний➡️')
        markup2.row('/3 дня предсказаний➡️')
        markup2.row('/Меню')
...
def predict_model(message):
    print(message.text)
    if  message.text=='/Ставрополь 3 дня предсказаний➡️' :
        print('here')
        global personal_info
       ...
       # n_window=30
        ...
        markup =telebot.types.ReplyKeyboardMarkup(True)
        markup.row('/Меню')
        bot.send_message(message.chat.id, 'Извиниете, Вы не приобрели данный чат-бот😥.\n Вы пока не можете загружать свои данные',reply_markup=markup)
